...
static FILE *readZippedFile(char *command, char *fileName) {
  char buf[4096], escapedfilename[4096];
  escapeSingleQuotes(fileName, escapedfilename);
  int n = snprintf(buf, sizeof(buf), "%s < '%s' 2>/dev/null", command, escapedfilename);
  if (n < 0 || n >= sizeof(buf)) {
    fprintf(stderr, "WARNING: Command string truncated in readZippedFile\n");
  }
...
FILE *readFile(char *fileName) {
  ...
  /* Try adding .gz */
  if (snprintf(fileBuf, sizeof(fileBuf), "%s.gz", fileName) < (int)sizeof(fileBuf)) {
    if (!stat(fileBuf, &statbuf))
      return readZippedFile(UNZIP, fileBuf);
  }
  /* Try adding .Z */
  if (snprintf(fileBuf, sizeof(fileBuf), "%s.Z", fileName) < (int)sizeof(fileBuf)) {
    if (!stat(fileBuf, &statbuf))
      return readZippedFile(UNZIP, fileBuf);
  }
  /* Try adding .bz2 */
  if (snprintf(fileBuf, sizeof(fileBuf), "%s.bz2", fileName) < (int)sizeof(fileBuf)) {
    if (!stat(fileBuf, &statbuf))
      return readZippedFile(BUNZIP2, fileBuf);
  }
  /* Try adding .bz */
  if (snprintf(fileBuf, sizeof(fileBuf), "%s.bz", fileName) < (int)sizeof(fileBuf)) {
    if (!stat(fileBuf, &statbuf))
      return readZippedFile(BUNZIP2, fileBuf);
  }
...
static FILE *writeZippedFile(char *fileName, flag append) {
  ...
  if (stringEndsIn(fileName, ".bz2") || stringEndsIn(fileName, ".bz"))
    n = snprintf(buf, sizeof(buf), "%s %s '%s'", BZIP2, op, escapedfilename);
  else if (stringEndsIn(fileName, ".lz4"))
    n = snprintf(buf, sizeof(buf), "%s %s '%s'", LZ4ZIP, op, escapedfilename);
  else if (stringEndsIn(fileName, ".zst"))
    n = snprintf(buf, sizeof(buf), "%s %s '%s'", ZSTDZIP, op, escapedfilename);
  else if (stringEndsIn(fileName, ".Z"))
    n = snprintf(buf, sizeof(buf), "%s %s '%s'", COMPRESS, op, escapedfilename);
  else
    n = snprintf(buf, sizeof(buf), "%s %s '%s'", ZIP, op, escapedfilename);
  if (n < 0 || n >= sizeof(buf)) {
    fprintf(stderr, "WARNING: Command string truncated in writeZippedFile\n");
  }
