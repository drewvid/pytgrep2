...
/* Initial Values */
#define INIT_HASH     500000
#define INIT_BUFFER   32768
#define INIT_TREES    20000
#define INIT_STREES   16000
#define PATTERN_BUF   512
#define INIT_KIDS     64
#define INIT_NAMES    64
#define INIT_PATTERNS 64
...
FILE *openCorpusFile(char *filename) {
  ...
  for (i = 0; i < NumWords; i++) {
    ...
    clearString(N);
    for (k = 0; k < j; k++) {
      if (!P) fatalError("Corrupted corpus lexicon format.");
...
void buildSentence(Sentence S, FILE *file) {
  ...
  if (fread(&trees, 1, sizeof(unsigned short), file) != sizeof(unsigned short)) {
    fatalError("Failed to read number of trees from corpus file.");
  }
